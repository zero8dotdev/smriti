...
/**
 * Parse a single Copilot session into a ParsedSession.
...
export async function parseCopilotSession(
  ...
  const parsedMessages = parseCopilotJson(content);
...
export async function ingestCopilot(
  ...
  for (const session of sessions) {
    ...
    try {
      // Use pure parser
      const parsed = await parseCopilotSession(session.sessionId, session.filePath);
      ...
      for (const msg of parsed.messages) {
        await addMessage(db, session.sessionId, msg.role, msg.plainText, { title: parsed.session.title });
      }
      ...
      if (onProgress) {
        onProgress(`Ingested ${session.sessionId} (${parsed.messages.length} messages) — project: ${projectId}`);
      }
    } catch (err: any) {
